...
def _wrap_com(obj):
    if obj is None:
        return None
    try:
        if isinstance(obj, sws.win32com.client.CDispatch):
            return obj
        return sws.win32com.client.Dispatch(obj)
    except Exception:
        return obj
...
def _operations2(body_a, body_b, op_type: int = SW_BODY_INTERSECT):
    ...
    except (TypeError, ValueError, AttributeError, sws.pythoncom.com_error):
